...
function DiagnoseContent() {
  ...
  const handleGoToRedesign = () => {
    if (!result) return;
    const diagUrl = searchParams.get('url') ?? '';
    const params = new URLSearchParams({
      ...
      is_redesign: 'true',
      diag_url: diagUrl,
      diag_score: String(result.score),
    });
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '48px 20px 100px' }}>
        ...
        {!loading && error && (
          <div style={{ textAlign: 'center', paddingTop: 60 }}>
            ...
            <button onClick={() => router.push('/')} style={{ padding: '14px 28px', background: 'linear-gradient(135deg,#c8a84a,#e8d48a)', border: 'none', borderRadius: 8, color: '#000', fontWeight: 900, cursor: 'pointer', fontSize: '0.95rem' }}>
              TOPへ戻る
            </button>
          </div>
        )}
        ...
        {!loading && result && (
          <div className="fade-up">
            ...
            <div style={{ background: bg2, border: '1px solid rgba(200,168,74,0.2)', borderRadius: 16, padding: '28px 24px', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {result.problems?.map((p, i) => (
                  <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', background: bg3, border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '14px 16px' }}>
            ...
            <div style={{ background: bg2, border: '1px solid rgba(200,168,74,0.2)', borderRadius: 12, padding: '20px', marginBottom: 28 }}>
              ...
              <p style={{ fontSize: '0.68rem', color: '#444', marginTop: 10 }}>このプロンプトでモックアップLPを自動生成します</p>
            </div>

            <button
              onClick={handleGoToRedesign}
              style={{ width: '100%', padding: '18px', background: 'linear-gradient(135deg,#c8a84a,#e8d48a)', border: 'none', borderRadius: 10, color: '#000', fontSize: '1rem', fontWeight: 900, cursor: 'pointer', marginBottom: 10 }}
            >
              このサイトをリデザインする →
            </button>
            <button
              onClick={() => router.push('/lp/new')}
              style={{ width: '100%', padding: '13px', background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, color: muted, fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'inherit' }}
            >
              新規LPを作成する
            </button>
          </div>
